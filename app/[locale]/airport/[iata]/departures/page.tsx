...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = t('departures_title', { airport: getAirportName(airport.iata, locale, airport.name), iata: airport.iata, city: airport.city });
  const description = t('departures_description', { airport: getAirportName(airport.iata, locale, airport.name), iata: airport.iata, city: airport.city });
...
export default async function DeparturesPage({ params }: Props) {
  ...
  const canonical = `${BASE}/${locale}/airport/${airport.iata}/departures`;
  const t = await getTranslations({ locale, namespace: 'meta' });
  const h1 = t('departures_title', { airport: getAirportName(airport.iata, locale, airport.name), iata: airport.iata, city: airport.city });
  ...
  return (
    <>
      ...
      <FlightBoard airport={airport} locale={locale} defaultMode="departures" displayName={getAirportName(airport.iata, locale, airport.name)} />
    </>
  );
}
