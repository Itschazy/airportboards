...
export default async function DeparturesPage({ params }: Props) {
  ...
  const h1At = getBoardFetchedAt(airport.iata, 'departures');
  const h1Fresh = h1At != null && Date.now() - h1At <= STALE_AFTER_H1;
  const h1 = t(h1Fresh ? 'departures_title' : 'departures_title_stale', { airport: name, iata: airport.iata, city, showCity });
  const desc = t(h1Fresh ? 'departures_description' : 'departures_description_stale', { airport: name, iata: airport.iata, city });
