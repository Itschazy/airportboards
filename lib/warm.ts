 ...
 */
/**
 * ХВОСТ УПЛОТНЁН ВТРОЕ 04.09.2026, потому что план простаивал на три четверти.
 *
 * Сроки писались под план в 100 000 запросов и после перехода на миллион остались прежними.
 * Замер: прогрев брал 243 720 в месяц при разрешённых ему 650 000 — он тратит не «сколько
 * дадут», а сколько требуют сроки, и останавливался на четверти плана. Простаивало ~743 000
 * запросов в месяц, за которые уже заплачено.
 *
 * ПОЧЕМУ ИМЕННО ХВОСТ, А НЕ ВСЁ ПОДРЯД. У mega и hub источников свежести ДВА: этот прогрев и
 * покупка на приходе живого читателя (freshnessWorthBuying пускает только их и закреплённых).
 * У остальных 2 657 аэропортов источник ОДИН — вот этот список. Читатель, пришедший из поиска
 * на страницу яруса small, видел борт суточной давности, где все рейсы уже вылетели, и купить
 * себе свежесть не мог никак. Поэтому вся прибавка ушла туда, где она единственная:
 *
 *     major  6 ч → 2 ч      mid  12 ч → 6 ч      small  24 ч → 12 ч
 *
 * Цена: прогрев 292 950 → 556 500 в месяц, 56% плана вместо 29%. Это 86% собственного потолка
 * прогрева (warmShare = 650 000); людской резерв в 350 000 не тронут. Денег не стоит вовсе —
 * тариф фиксированный, поминутных лимитов у ключа нет (limits_by_minute: 0).
 *
 * ПОЧЕМУ НЕ ЕЩЁ ЧАЩЕ. Первая редакция ставила mid 4 ч и small 8 ч — 705 250 в месяц, то есть
 * 109% потолка прогрева. Тик просто не выдал бы столько, и система встала бы в режим
 * «спроса больше бюджета» НАВСЕГДА. А это ровно тот режим, где взвешивание по ярусам (WEIGHT
 * ниже) начинает резать очередь, и режет оно с хвоста — то есть уплотнение хвоста обернулось
 * бы его же голоданием. Пока спрос МЕНЬШЕ бюджета, очередь не режется вовсе и младший ярус
 * получает свой срок целиком.
 *
 * Длительность тика ~773 аэропорта по 120 мс ≈ 93 с — вчетверо меньше допуска TICK_LAG_MS.
 *
...
const DEMAND_PINNED = new Set([
  ...
  // Добавлены 16.08 по МЕТРИКЕ (110112198, 9 579 визитов за 30 дней на 26 аэропортов) —
  // другой источник, чем у предыдущей волны, и это важно: Вебмастер меряет ПОКАЗЫ, Метрика
  // меряет тех, кто дошёл. Списки почти совпали, и расхождение как раз в этих четырёх.
  //
  // Правило отбора то же: аэропорт в топе по измеренному спросу И его ярусный норматив хуже
  // шести часов. Замер боевых бортов в 02:00 МСК: OSS сутки, CXR сутки, SSH сутки, LBD ТРОЕ
  // суток — то есть втрое хуже собственного норматива. У всех четырёх на табло «все рейсы уже
  // вылетели», а это 423 визита в месяц, приходящих на заведомо мёртвую страницу.
  //
  // SAW, HRB и SYX в топе тоже есть и тоже хуже шести часов по нормативу (ярус hub, 12 ч), но
  // НЕ закреплены намеренно: замер застал их на пяти часах, то есть внутри норматива. Тратить
  // на них бюджет ради двукратного запаса, которого они и так достигают, нечем оправдать.
  //
  // Цена: четыре аэропорта с суточного норматива на шестичасовой ≈ +20 запросов в сутки из
  // 2 933 доступных, 0.7%. Общий расход НЕ растёт: тик тратит ровно tickBudget().
  'OSS', 'CXR', 'SSH', 'LBD',

  // DLM переехал сюда из DEMAND_NOT_PINNED 16.08: там он стоял с обоснованием «замер 1 ч —
  // прогрев доходит и без закрепления», и следующий же замер застал его на СУТОЧНОМ борте,
  // где всё уже вылетело. Обоснование было построено на одиночном снимке и прожило один день.
  //
  // По правилу закрепление ему подходит, в отличие от SAW/HRB/SYX: ярус major, 53 рейса в
  // сутки — это несколько часов расписания на борт, а не полчаса, поэтому шестичасовой снимок
  // у него содержит предстоящие рейсы, а не только историю.
  'DLM',

  // Добавлены 31.08 — их нашла НЕ рука, а scripts/check-warm-demand.mjs, ровно для этого
  // и заведённая. За пятнадцать дней спрос перекинулся на другие города, список отстал, и
  // проверка это назвала: «в топ-25 спроса норматив хуже 6 ч и не объяснён у 2».
  //
  // OVB — теперь САМЫЙ востребованный кластер запросов сайта: «толмачево онлайн табло» и
  // «онлайн табло новосибирск» дали 13 489 показов за неделю 24–30.08 против 3 307 у Казани,
  // рост +384% и +579%. Борт при этом стоял СУТКИ и целиком в прошлом.
  // HRG — Хургада, +421% визитов за две недели, тот же суточный борт в прошлом.
  //
...
/**
 * Аэропорты, которые в топе спроса ЕСТЬ и закреплены быть НЕ должны — с причиной.
 *
 * Существует ради проверки scripts/check-warm-demand.mjs. Та сверяет список закреплений с
 * живым спросом из Метрики и падает, когда востребованный аэропорт получает норматив хуже
 * шести часов. Без этого набора она падала бы на каждом осознанном исключении, и её быстро
 * научились бы игнорировать — а проверка, которую игнорируют, хуже отсутствующей.
 *
 * Решение о том, кого не закреплять, принимается ЗДЕСЬ, рядом с самим списком, а не в
 * проверке. Проверка, которая заводит собственный предикат, спорит с кодом вместо того чтобы
 * его стеречь; на этом уже спотыкались трижды в check-sitemap-scope.
 */
export const DEMAND_NOT_PINNED: Record<string, string> = {
  /**
   * ОБОСНОВАНИЕ ЗДЕСЬ — ПО ПРАВИЛУ, А НЕ ПО ЗАМЕРУ. Первая версия этого набора объясняла
   * исключения одиночными снимками («замер 5 ч», «замер 1 ч»), и один из них был опровергнут
   * в тот же день: DLM, про который стояло «прогрев доходит и без закрепления», через сутки
   * стоял на суточном борте, где всё уже вылетело. Снимок в комментарии протухает быстрее,
   * чем его успевают прочитать; причина должна быть такой, чтобы её нельзя было опровергнуть
   * следующим замером.
   *
   * Правило для этих трёх: закрепление им НЕ ПОМОЖЕТ, и это арифметика, а не осторожность.
   * Борт отдаёт до 50 строк, а на загруженном аэропорту пятьдесят рейсов — это полчаса
   * расписания (замер 16.08: IST 0.5 ч, DXB 0.8 ч, PVG 0.6 ч, SAW 4.5 ч). Снимок, взятый
   * шесть часов назад, целиком лежит в прошлом независимо от того, шесть часов ему или
   * двенадцать. Платить за вдвое более частый прогрев ради борта, который всё равно будет
   * просроченным, нечем оправдать.
   *
   * Живого посетителя это не касается: он проходит через /api/flights и докупает свежую
   * выборку из человеческого резерва — ровно та развилка, ради которой резерв и существует
   * (см. примечание к TIERS). Просроченный снимок здесь видит краулер, а не человек.
   *
   * Для регионального аэропорта арифметика обратная и потому закрепление там работает: у KZN
   * 24 строки покрывают 15.5 часа расписания, у UFA 28 строк — 11.5 часа. Пятичасовой борт
   * там полон предстоящих рейсов, и ни один из них не показывает «всё уже вылетело».
   */
  /**
   * ALA добавлен 21.09.2026 — и по ПРОТИВОПОЛОЖНОЙ причине, чем три следующих.
   *
   * Те не закрепляют, потому что закрепление им не поможет; этот — потому что оно ему уже не
   * нужно. После уплотнения ярусов (см. TIERS) норматив major стал 1.5 ч, а закрепление даёт
   * срок mega, то есть 1 ч. Разница в тридцать минут не меняет содержимое борта: у Алматы 92
   * рейса в сутки, значит 50 строк покрывают около тринадцати часов расписания, и снимок
   * полуторачасовой давности полон предстоящих рейсов — «всё уже вылетело» там невозможно.
   *
   * Это арифметика, а не снимок, и потому не протухнет: пока у аэропорта десятки рейсов в
   * сутки, а норматив яруса измеряется часами, закрепление выигрывает минуты.
   */
  ALA: 'ярус major, 92 рейса/сут; после уплотнения норматив 1.5 ч, закрепление дало бы 1 ч — тридцать минут не меняют борт',

  SAW: 'ярус hub; 50 строк = 4.5 ч расписания — закрепление не спасёт от просрочки',
  HRB: 'ярус hub; окно борта того же порядка — закрепление не спасёт от просрочки',
 ...
 *
 * Заслон против фермы обходчиков, найденной в Google Analytics 04.09. Сигнатура в реальном
 * времени не оставляла сомнений: 263 активных, из них 256 первых визитов, 266 просмотров на
 * 256 РАЗНЫХ страниц — каждый «пользователь» открывает одну страницу, свою собственную, и
 * больше не возвращается. Источник указан у одного из 263. Страницы случайные по всему
 * корпусу и по всем языкам: DQM (en), HGO (de), OSI (es), MXP (ru). За 28 дней это же видно
 * в разрезе каналов: Direct 5 521 сессия при вовлечённости 11% и шести секундах, Unassigned
 * 2 706 при нуле — против Organic Search 13 093 при 82% и 55 секундах.
 *
 * Ферма исполняет наш JS (иначе не попадала бы в счётчик), то есть выглядит браузером и
 * проходит слой 1. Слой 2 считает ШИРИНУ на адрес, и ферма с ротацией адресов обходит и его.
 *
 * ПОЧЕМУ ЭТО СТАЛО ОПАСНО ИМЕННО СЕЙЧАС. До 04.09 живой путь не покупал данные вообще
 * (людская доля схлопнулась вдвое, см. lib/live-budget.ts), и ферма ничего не стоила. Как
 * только покупку починили, 256 разных бортов за полчаса превратились в ~12 000 оплаченных
 * запросов в сутки — треть месячного плана, полностью впустую.
 *
 * Разделитель выбран по ФОРМЕ спроса, а не по поведению: настоящий читатель приходит из
 * поиска на аэропорт, который ищут, — а ищут крупные и те, что закреплены по измеренному
 * спросу. Ферма идёт по карте сайта подряд, и в этот набор попадает лишь ~7% её обращений.
 *
 * Всё остальное продолжает отдаваться из хранилища — ровно как до 04.09, то есть это не
 * ухудшение против прежнего поведения ни для одной страницы.
 */
export function freshnessWorthBuying(iata: string): boolean {
  const code = iata.toUpperCase();
  if (DEMAND_PINNED.has(code)) return true;
  const t = tierOf(getServiceData()[code] ?? 0);
  return t?.name === 'mega' || t?.name === 'hub';
}
...
export function dueAirports(now = Date.now()): Due[] {
  ...
  for (const [iata, flights] of Object.entries(svc)) {
     ...
     */
    /**
     * Поднято 04.09 с пяти минут до пятнадцати вместе с уплотнением ярусов.
     *
     * Допуск обязан покрывать ВСЮ длительность тика: запись ложится после ответа поставщика,
     * а аэропорты идут со сдвигом 120 мс. Пять минут рассчитывались на тик примерно в сотню
     * аэропортов. После уплотнения ярусов и подъёма плана бюджет тика вырос до ~2 000, то есть
     * последние записи ложатся уже минуты через четыре после старта — на самой границе. Кто не
     * уложился, промахивается мимо своего юбилейного тика и ждёт лишние два часа.
     *
     * Пятнадцать минут по-прежнему далеко от самого короткого норматива (120 минут), так что
     * двойного прогрева допуск вызвать не может.
     */
    const TICK_LAG_MS = 15 * 60_000;
    const age = now - ts;
