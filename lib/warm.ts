...
export const DEMAND_NOT_PINNED: Record<string, string> = {
  // Ярус hub даёт им 12 ч, и замер 16.08 в 02:00 МСК застал все три на пяти часах — внутри
  // норматива с двукратным запасом. Закреплять значит платить за свежесть, которой они и так
  // достигают. Пересмотреть, если замер начнёт заставать их за пределами двенадцати часов.
  SAW: 'ярус hub, 12 ч, замер 5 ч — внутри норматива',
  HRB: 'ярус hub, 12 ч, замер 5 ч — внутри норматива',
  SYX: 'ярус hub, 12 ч, замер 5 ч — внутри норматива',
  // Ярус major, норматив суточный, но замер застал борт на часе: прогрев доходит без
  // закрепления. 64 визита в месяц — самый хвост выборки.
  DLM: 'ярус major, замер 1 ч — прогрев доходит и без закрепления',
};
