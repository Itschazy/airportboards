 ...
 */
/**
 * Нужна ли покупка вообще — или борт ещё внутри окна свежести.
 *
 * Отдельная функция, потому что спрашивать надо ДО mayFetchLiveFor, а не внутри. Внутри поздно:
 * mayFetchLiveFor под нормальным бюджетом пропускает не глядя на возраст (withinHumanReserve
 * истинно), и по дороге СЪЕДАЕТ слот из потолка по адресу — сорок разных бортов в час. Дальше
 * fetchRaw всё равно вернёт запись из хранилища бесплатно (getFresh стоит первым), денег не
 * тратится ни копейки, но слот уже потрачен.
 *
 * Кому это мешало: живому человеку, который смотрит много аэропортов подряд. Он выбирал свои
 * сорок слотов на заходах, которые ничего не стоили, а потом получал отказ на борте, который
 * купить действительно следовало. Плюс счётчик admitted завышал число покупок, то есть врал
 * ровно там, где по нему судят о нагрузке.
 *
 * Окно то же самое, что у хранилища (FLIGHT_TTL_SEC, по умолчанию 600 с) — не копия числа, а
 * та же переменная, поэтому разъехаться они не могут.
 */
export function needsRefresh(ageMs: number | null): boolean {
...
function freshnessOverride(ageMs: number | null): boolean {
  ...
  /**
   * Потолок меряется по ЛЮДСКОМУ счётчику, а не по общему, и это исправление 04.09.
   *
   * Прежде здесь стояло `usage().count < …`, то есть право на свежесть упиралось в СУММУ
   * трат обеих сторон. Следствие: живой путь мог выесть общий счётчик до 65% плана, а
   * tickBudget останавливает прогрев ровно там же (`remaining - reserve` обнуляется). Пол,
   * названный «долей, которую нельзя отнять у прогрева», не защищал прогрев ни от чего —
   * при достаточном трафике прогрев получал НОЛЬ, и первый посетитель каждой страницы снова
   * видел бы вчерашнее. Ферма обходчиков делает такой трафик достижимым, а не гипотетическим.
   *
   * Теперь у каждой стороны свой счётчик и своя доля: люди до 65% плана, прогрев до 65%
   * своих (см. tickBudget). Общий жёсткий предел держит canSpend() — обе доли вместе выйти
   * за план не могут.
   */
  return usage().human < monthlyCap() * (1 - WARM_FLOOR_PCT);
}
