 ...
 */
/**
 * Знаки, которые встречаются только в традиционном письме. Список неполный намеренно: он
 * нужен лишь чтобы УЗНАТЬ пару «традиционное / упрощённое» и не трогать её автоматически,
 * а не чтобы конвертировать. Полная конвертация — задача OpenCC, и угадывать её тут нечем.
 */
const TRADITIONAL = /[臺灣爾諾馬納東龍學國電話產業麼個們來時萬與並豐劃區華萬鳥點聯絡舊邊營選]/;
...
for (const file of FILES) {
  ...
  for (const [key, locs] of Object.entries(data)) {
    ...
    //
    // Два условия, каждое из которых я нарушил в первой версии и получил дефект на проде:
    //
    //   1. слэш ВНУТРИ скобок разрезать нельзя. «帕扑阿普·基里汉（帕府/帕州）» превратилось в
    //      «帕扑阿普·基里汉（帕府» — незакрытая скобка уехала в <title> живой страницы;
    //   2. пара «традиционное / упрощённое» — не две транслитерации, а одно и то же имя двумя
    //      письменностями, и для локали zh (упрощённые знаки) выбирать надо ВТОРОЕ.
    //      «臺中 / 台中» стало «臺中», «馬格特/马格特» — «馬格特».
    //
    //   3. двойное имя — не глосса. «威尔克斯-巴里/斯克兰顿» это Wilkes-Barre/Scranton, два
    //      РАЗНЫХ города, которые обслуживает один аэропорт. Проверки по английскому ключу
    //      здесь мало: ключ аэропорта это «AVP», слэша в нём нет. Различает их тот же признак,
    //      что и в скобках, — ПОХОЖЕСТЬ: две транслитерации одного имени делят иероглифы,
    //      два разных города не делят ничего.
    if (/\s*\/\s*/.test(next) && !/\//.test(key) && !/[（(]/.test(next)) {
      const parts = next.split(/\s*\/\s*/).filter(Boolean);
      // Порог здесь ВЫШЕ, чем для скобок (0.6 против 0.25), и по измеренной причине: знаки
      // 斯 и 克 стоят почти в каждой китайской транслитерации, поэтому «威尔克斯-巴里» и
      // «斯克兰顿» — два разных города — делят половину знаков и на мягком пороге слипались
      // в один. Настоящая пара написаний делит больше: «阿尔胡塞马/阿尔侯西马» — 0.6,
      // «哈尔季/哈尔吉» — 0.67.
      const alike = parts.length > 1 && overlap(parts[0], parts.slice(1).join('')) >= 0.6;
      if (alike && !parts.some((x) => TRADITIONAL.test(x))) {
