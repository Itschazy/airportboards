...
if (!token) {
  ...
  /**
   * ТОЛЬКО ПОИСКОВЫЙ ТРАФИК. Фильтр появился 05.09.2026 и он не косметический.
   *
   * Ферма обходчиков дошла до русской локали: 02.09 — 370 визитов при 14% отказов и среднем
   * визите 147 с; 05.09 — 2 404 визита, 83% отказов, 33 с. Разбор: Сингапур, «прямые заходы»,
   * 500+ РАЗНЫХ страниц за сутки, медиана ОДИН визит на страницу. Он идёт по корпусу подряд и
   * не возвращается.
   *
   * Чем это опасно ИМЕННО ЗДЕСЬ. Спрос считается по посадочным страницам, а закрепление режет
   * норматив прогрева тем, кто в топе. Ферма кладёт ровный пол примерно в 4 визита на страницу
   * в сутки — это 120 за тридцатидневное окно, а порог входа в топ-25 на 05.09 равен 141.
   * То есть она уже почти дотянулась, а объём её утраивается за сутки. Ещё немного — и прогрев
   * начал бы держать свежими страницы, которые смотрят боты, за счёт тех, что смотрят люди.
   * Заметить это было бы нечем: цифры Метрики настоящие, просто визиты не человеческие.
   *
   * Органика — правильный отбор не только против фермы. 99.7% показов сайта приходят из
   * Яндекса, и закрепление существует ради человека, пришедшего из поиска. Прямые заходы до
   * фермы давали 24 визита против 249 поисковых, то есть отбрасываем меньше десятой части, и
   * та часть — не тот спрос, ради которого это считается.
   */
  const ORGANIC_ONLY = "ym:s:lastTrafficSource=='organic'";
  const qs = new URLSearchParams({
    ids: '110112198', metrics: 'ym:s:visits', dimensions: 'ym:s:startURLPathLevel4',
    date1: from, date2: to, limit: '200', sort: '-ym:s:visits', filters: ORGANIC_ONLY,
  });
  try {
