...
for (const locale of LOCALES) {
  ...
  for (const path of PATHS) {
    ...
    if (locale === 'ar') {
      ...
      const notYear = west.filter((n) => !/^20\d\d$/.test(n));
      if (/[٠-٩]/.test(text) && notYear.length) {
        fail(locale, `${path || '/'}: западные цифры рядом с восточноарабскими — ${notYear.slice(0, 3).join(' ')}`);
      }
    }
    ok++;
  }
