  ...
  async headers() {
    return [
      ...
      },
      {
        ...
        /**
         * ⚠️ Картинка для соцсетей ИСКЛЮЧЕНА, и это не мелочь.
         *
         * Правило писалось для табло, но `:path*` накрыл и `/airport/KZN/opengraph-image` —
         * а её рисует satori, 4.2 секунды холодного рендера на замере прода. Запретив ей
         * кэш, я заставил каждого обходчика соцсетей и поиска перерисовывать PNG заново.
         * Живых данных в ней нет вовсе: код, имя аэропорта и город.
         *
         * Отрицательный просмотр в параметре — единственная форма, при которой правило не
         * накроет её снова: перечислять исключения списком значило бы забыть следующее.
         * Не подошедшая под это правило картинка попадает под общее правило локали выше и
         * получает `public, max-age=300, s-maxage=3600` — как у событий, где всё верно.
         */
        source: '/:locale(en|ru|zh|ar|de|ko|ja|fr|es|it|hi|tr)/airport/:path((?!.*opengraph-image).*)',
        headers: [
          /**
           * Табло НЕ КЭШИРУЕТСЯ — ни у читателя, ни в общем кэше.
           *
           * Прежнее `public, max-age=60, s-maxage=300` сводило на нет весь смысл живого
           * рендера: страница честно отрисовывалась на каждый запрос, а заголовок разрешал
           * отдать её копию следующим пяти минутам посетителей. Свежесть на первом экране и
           * разрешение кэшировать ответ несовместимы by construction.
           *
           * `max-age=0` вместо `no-store` намеренно: `no-store` запрещает и
           * back/forward-кэш, то есть возврат кнопкой «назад» перерисовывал бы страницу с
           * нуля. `must-revalidate` заставляет спросить сервер, но позволяет браузеру
           * показать прежний экран, пока идёт запрос.
           *
           * Правило накрывает /airport/:iata и его подстраницы — ровно те, где живёт борт.
           * Остальные типы страниц (страны, города, указатель) кэшируются как раньше:
           * их содержимое меняется раз в сутки, и платить за них рендером незачем.
           */
          { key: 'Cache-Control', value: 'private, max-age=0, must-revalidate' },
        ],
      },
      {
        // Карта сайта. Next выводит ей `s-maxage=31536000` — год, — тот же класс мины, что
        // был на страницах локалей: сегодня общего кэша нет и число не значит ничего, а с
        // приходом CDN карта замёрзнет в том виде, в каком её однажды забрали, и новые
        // аэропорты в неё не попадут до следующего года.
        //
        // Сутки: корпус меняется медленно, но не раз в год, а роботы карту перечитывают
        // регулярно и заслуживают актуальную.
        // Два правила, а не одно `/sitemap:path*`: параметр без слеша перед ним Next
        // молча ОТБРАСЫВАЕТ — правило не попадает в собранный routes-manifest.json
        // вовсе, и на сборке об этом никто не сообщает.
        source: '/sitemap.xml',
        headers: [
